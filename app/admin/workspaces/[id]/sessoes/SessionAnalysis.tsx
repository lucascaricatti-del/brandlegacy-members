          ...
          >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-text-muted mt-0.5">
                  ...
                  {session.session_tasks.length > 0 && (
                    <span className="ml-2">
                      {session.session_tasks.length} tarefa(s)
                    </span>
                  )}
                </p>
              </div>
            ...
            {isExpanded && (
              <div className="px-5 pb-5 space-y-5 border-t border-border">
                {/* Transcript input (only if pending/error) */}
                {(session.status === 'pending' || session.status === 'error') && (
                  <div className="pt-4 space-y-3">
                    ...
                    <textarea
                      ...
                      rows={8}
                      placeholder="Cole aqui a transcrição completa da reunião de mentoria..."
                      className="
                        w-full px-4 py-3 rounded-lg bg-bg-base border border-border
                      ...
                      <div>
                        <div className="flex items-center justify-between mb-3">
                          <h4 className="text-xs font-semibold text-text-secondary uppercase tracking-wide">
                            Tarefas Extraídas ({session.session_tasks.length})
                          </h4>
                          {pendingTasks.length > 0 && (
                            <button
                              onClick={() => handleAddAllToKanban(session.id)}
                              disabled={isPending || loadingAction === `all-${session.id}`}
                              className="
                                flex items-center gap-1.5 px-3 py-1.5 rounded-lg
                                bg-brand-gold/15 text-brand-gold border border-brand-gold/30
                                text-xs font-medium hover:bg-brand-gold/25 transition-colors
                                disabled:opacity-60
                              "
                            >
                              {loadingAction === `all-${session.id}` ? (
                                'Adicionando...'
                              ) : (
                                <>
                          ...
                          {session.session_tasks.map((task) => (
                            ...
                            >
                              ...
                              {task.kanban_card_id ? (
                                ...
                                <button
                                  onClick={() => handleAddToKanban(task.id)}
                                  disabled={isPending || loadingAction === `task-${task.id}`}
                                  className="
                                    flex items-center gap-1 px-2.5 py-1.5 rounded-lg
                                    bg-brand-gold/10 text-brand-gold border border-brand-gold/20
                                    text-[11px] font-medium hover:bg-brand-gold/20 transition-colors
