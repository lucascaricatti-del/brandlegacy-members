...
export default function NewSessionForm({ workspaceId }: Props) {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const result = await createSession(workspaceId, title, sessionDate)
      if ('error' in result) {
  ...
  return (
    ...
    >
      <h3 className="text-sm font-semibold text-text-primary">Nova Sessão de Mentoria</h3>
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          disabled={isPending}
          className="
            px-4 py-2 rounded-lg bg-brand-gold text-bg-base
            text-sm font-medium hover:bg-brand-gold-light
