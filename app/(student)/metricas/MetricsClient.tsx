...
export default function MetricsClient({
  workspaceId, isMetaConnected, metrics,
}: {
  workspaceId: string; isMetaConnected: boolean; metrics: AdsRow[]
}) {
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      else { setSyncMsg(`${data.synced} registros sincronizados. Recarregue a página.`) }
  ...
  if (!isMetaConnected) {
    return (
      <div className="bg-bg-card border border-border rounded-xl p-16 text-center">
        <p className="text-text-primary font-semibold text-lg mb-2">Meta Ads não conectado</p>
        <p className="text-text-muted mb-4">Conecte sua conta do Meta Ads para ver métricas.</p>
